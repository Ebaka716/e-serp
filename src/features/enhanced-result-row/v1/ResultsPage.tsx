...
export default function ResultsPage({ searchQuery }: ResultsPageProps) {
  ...
  const handleNewSearch = (query: string) => {
    router.push(`/enhanced-result-row/v1/results?q=${encodeURIComponent(query)}`);
  };
  ...
  return (
    <Layout variant="results">
      ...
      <button
        ...
        className="fixed bottom-6 left-6 z-50 bg-blue-600 text-white px-4 py-2 rounded-full shadow-lg hover:bg-blue-700 transition-colors"
        style={{ minWidth: 80 }}
      >
        {devMode ? 'Hide Labels' : 'Show Labels'}
      </button>
    </Layout>
  );
} 